defmodule Permit.LiveViewAuthorizeTest do
  ...
  defp mock_socket(assigns \\ %{}) do
    %Phoenix.LiveView.Socket{
      ...
      assigns: Map.merge(%{current_user: %User{id: 1, roles: ["admin"]}}, assigns)
    }
  end
  ...
  describe "authorize!/4" do
    test "executes function when resource exists and user is authorized" do
      ...
      socket = mock_socket(%{current_user: %User{id: 1, roles: [:admin]}})
    ...
    test "calls handle_unauthorized when user lacks permission" do
      ...
      socket = mock_socket(%{current_user: %User{id: 2, roles: [:inspector]}})
    ...
    test "raises error when current_user is nil" do
      socket = mock_socket(%{current_user: nil})
      resource = %Item{id: 1, owner_id: 1}
    ...
    test "preserves socket state when executing function" do
      socket =
        mock_socket(%{current_user: %User{id: 1, roles: [:admin]}, test_assign: "original_value"})
